...
class Program
{
	static void Main()
	{
		...
		using (FileStream stream = new FileStream("person.xml", FileMode.OpenOrCreate)) // rubah extension dari xml or json
		{
			serializer.WriteObject(stream, people);
		}

		List<Person> importPerson;
		using (FileStream stream2 = new FileStream("person.xml", FileMode.Open)) // rubah extension dari xml or json
		{
